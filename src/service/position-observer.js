...
export class AmpPagePositionObserver extends AbstractPositionObserver {
  ...
  startCallback() {
    ...
    this.unlisteners_.push(this.viewport_.onScroll(() => {
      ...
      this.schedulePass_();
      setTimeout(() => {
        if (Date.now() - this.scrollTimer_ < 1000) {
          // viewport scroll in the last 1000 ms, wait
          return;
        }
        // assume scroll stops, if no scroll event in the last 1000,
        this.inScroll_ = false;
      }, 1000);
    }));
  ...
  */
  ...
  updateEntryPosition_(entry) {
    ...
    // TODO: same as viewport.getSize() refactor?
...
//TODO: need something similar for FriendlyIframe since positions are relative
// to iframe window.
...
export class InaboxPositionObserver extends AbstractPositionObserver {
  ...
  constructor(ampdoc) {
    ...
    // Should context or at least sentinel be already available on an AMP page
    // when in a box?
